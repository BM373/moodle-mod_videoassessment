...
if (
    ...
    foreach ($formusers as $key => $userid) {
        if (empty($formtimings[$key]) || !in_array($formtimings[$key], ['before', 'after'])) {
            continue;
        }
        if (empty($userid)) {
            continue;
    ...
    redirect(new moodle_url($baseurl, ['cmid' => $cmid, 'changed' => $changed, 'error' => $error]));
...
if ($changed = optional_param('changed', 0, PARAM_INT)) {
    $viewurl = new moodle_url('/mod/videoassessment/bulkupload/view.php', ['cmid' => $cmid]);
    echo $OUTPUT->notification(
        html_writer::tag(
            'a',
            get_string('associated', 'videoassessment', $changed),
            ['href' => $viewurl, 'style' => 'font-size:150%;']
        ),
        'notifysuccess'
    );
}
...
foreach ($unassociatedfiles as $key => $file) {
    ...
    $btndelete = $OUTPUT->action_link(
        new moodle_url($baseurl, ['cmid' => $cmid, 'delete' => $file->get_id(), 'sesskey' => sesskey()]),
        $OUTPUT->pix_icon('t/delete', ''),
        null,
        ['onclick' => 'return confirm("' . get_string('reallydeletevideo', 'videoassessment') . '")']
    );
    ...
    $table->add_data(
        [
            ...
            html_writer::select($useropts, 'user[' . $key . ']', 0, [0 => '']),
            implode(' ', $radios),
...
echo html_writer::tag(
    'div',
    $OUTPUT->action_link(
        new moodle_url('/mod/videoassessment/bulkupload/view.php', ['cmid' => $cmid]),
